...
SELECT payment_id, amount, payment_date FROM payment WHERE payment_date >= STR_TO_DATE('2005-05-27 00:00:00', '%Y-%m-%d %H:%i:%s');

# 3c. Select the primary key, amount, and payment date columns from the payment table for payments made on 05/27/2005.
SELECT payment_id, amount, payment_date FROM payment WHERE payment_date <= STR_TO_DATE('2005-05-27 23:59:59', '%Y-%m-%d %H:%i:%s') AND payment_date >= STR_TO_DATE('2005-05-27 00:00:00', '%Y-%m-%d %H:%i:%s');
...
SELECT payment_id, payment_date, amount FROM payment WHERE amount > 5 LIMIT 51, 50;
...
SELECT title, description, special_features, length, rental_duration FROM film WHERE special_features < 120 AND rental_duration BETWEEN 5 AND 7 ORDER BY length DESC LIMIT 10;
...
SELECT film.title, film.description, film.release_year, language.name AS language FROM film LEFT JOIN language ON film.language_id = language.language_id;
...
SELECT staff.first_name, staff.last_name, address.address, address.address2, city.city, address.district, address.postal_code FROM staff LEFT JOIN address ON staff.address_id = address.address_id LEFT JOIN city ON address.city_id = city.city_id;
